...
export function useSaveAvailability() {
	...
	return useMutation({
		mutationFn: async ({
			...
			const { error: deleteError } = await deleteAvailabilityByParticipantAndDay(
				supabase,
				participantId,
				dayStart,
				dayEnd
...
export function useUpdateAvailability() {
	...
	return useMutation({
		mutationFn: async ({
			...
			const { error } = await updateAvailability(supabase, id, slotStart.toISOString(), slotEnd.toISOString());
			if (error) throw error;
			return { eventSlug };
		},
		onSuccess: (result) => {
