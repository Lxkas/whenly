...
export function MonthDayCell({ date, isCurrentMonth, isValid, availableParticipants, totalParticipants }: MonthDayCellProps) {
    ...
    return (
        <button
            ...
            disabled={!isValid}
            className={cn(
                "relative aspect-square p-1 rounded-md text-sm transition-colors select-none",
                "flex flex-col items-center justify-start",
                !isCurrentMonth && "text-muted-foreground/40",
                isValid && "hover:bg-accent cursor-pointer",
                !isValid && "opacity-50 cursor-not-allowed",
                isToday && "ring-1 ring-primary",
                isSelected && isValid && "bg-primary/20 ring-2 ring-primary"
            )}
        >
            <span
                className={cn(
            ...
            {hasAvailability && isValid && (
                <div className="absolute bottom-1 left-1 right-1 flex justify-center gap-0.5">
                    ...
                    {availableParticipants.length > 4 && (
                        <span className="text-[8px] text-muted-foreground">+{availableParticipants.length - 4}</span>
                    )}
                </div>
            )}
